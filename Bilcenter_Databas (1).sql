...
-- Skapa tabellen för reservdelar
CREATE TABLE Reservdelar (
    ReservdelID INT AUTO_INCREMENT PRIMARY KEY,
    Namn VARCHAR(100) NOT NULL,
    Kategori VARCHAR(50) NOT NULL,
    Pris DECIMAL(10,2) NOT NULL,
    Lagerstatus INT NOT NULL DEFAULT 0,
    FordonID INT,  -- Om reservdelen är specifik för en bilmodell
    FOREIGN KEY (FordonID) REFERENCES Bilar(FordonID) ON DELETE SET NULL
);
...
CREATE TABLE Reservdelar_i_Ordrar (
    ...
    Pris DECIMAL(10,2) NOT NULL,
    FOREIGN KEY (OrderID) REFERENCES Beställningar(OrderID) ON DELETE CASCADE,
    FOREIGN KEY (ReservdelID) REFERENCES Reservdelar(ReservdelID) ON DELETE CASCADE
);

DELIMITER $$

CREATE TRIGGER Uppdatera_Lagerstatus_Reservdel
AFTER INSERT ON Reservdelar_i_Ordrar
FOR EACH ROW
BEGIN
    UPDATE Reservdelar
...
-- Index på registreringsnummer för snabbare sökningar
CREATE INDEX idx_registreringsnummer ON Bilar (Registreringsnummer);


-- Index på epost för att snabba upp sökningar på kunder
...
-- Lägga till Reservdelar
INSERT INTO Reservdelar (Namn, Kategori, Pris, Lagerstatus, FordonID) VALUES
('Bromsskivor', 'Bromssystem', 1200.00, 10, 1),
('Oljefilter', 'Motor', 250.00, 30, 2),
('Tändstift', 'Elektronik', 100.00, 50, NULL);

-- Lägga till en reservdel i en order
INSERT INTO Reservdelar_i_Ordrar (OrderID, ReservdelID, Antal, Pris) VALUES
(1, 1, 2, 2400.00),  -- 2 bromsskivor för order 1
(2, 3, 4, 400.00);   -- 4 tändstift för order 2
